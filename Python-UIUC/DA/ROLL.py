...
def plus_build_portfolio(trade_list, trading_data , trade_cost=0.005):
    ...
    for aa in range(1, len(portfolio)):
        if portfolio.iloc[aa-1]['Short'] > 0:
            portfolio.iloc[aa]['ShortR'] = -(portfolio.iloc[aa]['Short'] - portfolio.iloc[aa-1]['Short'])/portfolio.iloc[aa-1]['Short']
            portfolio.iloc[aa]['LongR'] = (portfolio.iloc[aa]['Long'] - portfolio.iloc[aa-1]['Long'])/portfolio.iloc[aa-1]['Long']
        else:
...
idk.plot_histogram(bins = 100, title = 'Return histogram', color = 'DarkGreen', alpha=0.5)
...
date_index = Close.loc['2008-02-11':].index.values

all = pd.read_csv('all_ret.csv' , index_col = 'Unnamed: 0')

all.index = date_index

bbbbb = ((all['cnn']+1).cumprod())
bbbbb = bbbbb.calc_stats()

bbbbb.plot_histogram(title = 'Return histogram of CNN', color = 'DarkGreen', alpha=0.5)
plt.savefig('CNN.png')
